...
<?php
    ...
    if (isset($_POST['edit'])){
        ...
        $dbImage = $admin->getSliderById($id)['filename'];

//        echo "<script>alert('".$dbImage."')</script>";

        if (!file_exists($folder.$dbImage)){
//            $admin->editSlider($id, $name, $baseName, $date);
//            if (move_uploaded_file($file['tmp_name'], $folder.$baseName)){
//                echo "<script>alert('Edited File Error')</script>";
//            }else{
//                echo "<script>alert('Editing File Error')</script>";
//            }
            echo "<script>alert('ararsebobs')</script>";

        }else{
            if($admin->deleteImage($id)){
                if (move_uploaded_file($file['tmp_name'], $folder.$baseName)){
                    echo "<script>alert('Uploaded File')</script>";
                    $admin->editSlider($id, $name, $baseName, $date);
                }else{
                    echo "<script>alert('Uploading File Error')</script>";
                }
            }else{
        ...
        if (!file_exists($folder.$baseName)){
            if (move_uploaded_file($file['tmp_name'], $folder.$baseName)){
                if (substr($baseName, -3) == "svg"){
                    $admin->addSlider($title, $baseName, $date);
                }else{
    ...
    <div class="br-mainpanel">
        ...
        <div class="br-pagebody">
            <div class="row row-sm">
                <table class="table table-striped">
                    ...
                    <?php foreach ($admin->getSlider() as $slider) { ?>
                        <tr>
                            ...
                            <td><?=$slider['date'];?></td>
                            <td><a href="" class="edit" data-toggle="modal" data-target="#myModal2" data-id="<?=$slider['id'];?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td>
                            <td><a href="" class="delete" data-toggle="modal" data-target="#myModal3" data-id="<?=$slider['id'];?>"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
                                            ...
                                            <form class="form-horizontal" action="home.php" method="post" enctype="multipart/form-data">
                                                <div style="display: table; width: 50%; margin: 0 auto;">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="">
                                                            <input type="file" name="filename" class="form-control" id="inputFile">
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="inputFile" class="control-label">Date</label>
                                                        <div class="">
                ...
                <button class="add btn btn-default" data-toggle="modal" data-target="#myModal" >ADD</button>
            </div><!-- row -->
        </div><!-- br-pagebody -->
